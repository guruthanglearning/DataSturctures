...
namespace WindowsFormsApplication3
{
    public partial class Trees : Form
    {
        ...
        private void btn_Count_Good_Nodes_in_Binary_Tree_Click(object sender, EventArgs e)
        {
            /*
            ...
            StringBuilder result = new StringBuilder();
            List<Common> inputs = new List<Common>();
            inputs.Add(new Common() { Tree = this.CreateBinaryTreeFromArray(new int?[] { 3, 1, 4, 3, null, 1, 5 })});
            inputs.Add(new Common() { Tree = this.CreateBinaryTreeFromArray(new int?[] { 3, 3, null, 4, 2 })});
            inputs.Add(new Common() { Tree = this.CreateBinaryTreeFromArray(new int?[] { 1})});

            foreach (var input in inputs)
            {
                result.AppendLine($"Count Good Nodes in Binary Tree  for the given Binary tree is {this.GoodNodes(input.Tree)}");                
            }
        ...
        public int GoodNodes(Node root)
        {
            ...
            int count = 0;
            GetCount(root, ref count, int.MinValue);
            return count;

        }

        public void GetCount(Node root, ref int count, int maxValue)
        {
            if (root == null)
                return;
            ...
            if (root.left != null)
                GetCount(root.left, ref count, maxValue);
        ...
        private void btn_Verify_Preorder_Serialization_of_a_Binary_Tree_Click(object sender, EventArgs e)
        {
            /*
            ...
            foreach (var input in inputs)
            {
                result.AppendLine($"Verification of  Preorder Serialization of a Binary Tree {input}  is  {(this.IsValidSerialization(input) ? "true" : "false")}");
            }
